...
namespace Allure.Xunit.v3.Tests.MetaAttributes;

class MetaAttributeTests
{
    [Test]
    public async Task CheckMetaAttributeWorks(CancellationToken token)
    {
        var results = await AllureSampleRunner.RunAsync(AllureSampleRegistry.MetaAttributes, token);
        ...
        await Assert.That(results).HasSingleTestResult()
            ...
            .With.OnlyOneLink(link => link.HasUrl("https://foo.bar/").And.HasNoName().And.HasNoType());
    }
}
