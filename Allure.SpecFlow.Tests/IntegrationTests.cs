...
namespace Allure.SpecFlow.Tests
{
    [TestFixture]
    public class IntegrationFixture
    {
        ...
        public void Init()
        {
            ...
            var featuresDirectory = Path.Combine(samplesProjectDir, "TestData");

            // parse allure suites
            ParseAllureSuites(this.allureResultsDir.FullName);
        ...
        {
            if (this.allureResultsDir.Exists)
            {
                this.allureResultsDir.Delete(recursive: true);
            }
            if (this.allureConfigFile.Exists)
            {
