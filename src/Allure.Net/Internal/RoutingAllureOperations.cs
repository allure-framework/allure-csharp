...
class RoutingAllureOperations : IAllureInProcessOperations
{
    public void AddAttachment(string name, Stream content, string? mediaType, string fileExtension) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.AddAttachment(name, content, mediaType, fileExtension);

    public void AddFileAttachment(string name, string path, string? mediaType, string fileExtension) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.AddFileAttachment(name, path, mediaType, fileExtension);

    public void AddFileScreenDiff(string expectedPath, string actualPath, string diffPath) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.AddFileScreenDiff(expectedPath, actualPath, diffPath);

    public void AddGlobalAttachment(string name, Stream content, string? mediaType, string fileExtension) =>
        AllureBackend.RuntimeForGlobalScope?.Operations.Sync.AddGlobalAttachment(name, content, mediaType, fileExtension);

    public void AddGlobalError(GlobalError error) =>
        AllureBackend.RuntimeForGlobalScope?.Operations.Sync.AddGlobalError(error);

    public void AddGlobalFileAttachment(string name, string path, string? mediaType, string fileExtension) =>
        AllureBackend.RuntimeForGlobalScope?.Operations.Sync.AddGlobalFileAttachment(name, path, mediaType, fileExtension);

    public void AddLabel(Label label) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.AddLabel(label);

    public void AddLabels(IEnumerable<Label> labels) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.AddLabels(labels);

    public void AddLink(Link link) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.AddLink(link);

    public void AddLinks(IEnumerable<Link> links) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.AddLinks(links);

    public void AddScreenDiff(Stream expected, Stream actual, Stream diff) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.AddScreenDiff(expected, actual, diff);

    public void AddTestParameter(Parameter parameter) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.AddTestParameter(parameter);

    public void SetDescription(string description) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.SetDescription(description);

    public void SetDescriptionHtml(string descriptionHtml) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.SetDescriptionHtml(descriptionHtml);

    public void SetFixtureName(string newName) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.SetFixtureName(newName);

    public void SetLabel(string name, string value) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.SetLabel(name, value);

    public void SetName(string newName) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.SetName(newName);

    public void SetTestName(string newName) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.SetTestName(newName);

    public void SetUp(string name, IEnumerable<Parameter> parameters, Action body)
    {
        if (AllureBackend.RuntimeForCurrentScope is { } backend)
        {
    ...
    public void Step(string name, IEnumerable<Parameter> parameters, Status status, StatusDetails? statusDetails) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.Step(name, parameters, status, statusDetails);
    ...
    public void UpdateFixtureResult(Action<FixtureResult> update) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.UpdateFixtureResult(update);

    public void UpdateStepResult(Action<StepResult> update) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.UpdateStepResult(update);

    public void UpdateTestResult(Action<TestResult> update) =>
        AllureBackend.RuntimeForCurrentScope?.Operations.Sync.UpdateTestResult(update);
